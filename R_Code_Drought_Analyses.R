...
##Young Leaves
mm1<-clmm(YL~Season*Drought + (1|tree:spp) + (1|Month)+(1|Green.Year), 
          data = phen, na.action = na.exclude)
mm1.null<-clmm(YL~Season + (1|tree:spp) + (1|Month)+(1|Green.Year), 
               data = phen, na.action = na.exclude)
anova(mm1.null,mm1)

##Mature Leaves
mm2<-clmm(ML~Season*Drought + (1|tree:spp) + (1|Month)+(1|Green.Year), 
          data = phen, na.action = na.exclude)
mm2.null<-clmm(ML~Season + (1|tree:spp) + (1|Month)+(1|Green.Year), 
               data = phen, na.action = na.exclude)
anova(mm2.null,mm2)

##Ripe Fruit
mm3.null<-clmm(Ripe.fruit~Season + (1|tree:spp) + (1|Month)+(1|Green.Year), 
               data = phen, na.action = na.exclude)
mm3<-clmm(Ripe.fruit~Season*Drought + (1|tree:spp) + (1|Month)+(1|Green.Year), 
          data = phen, na.action = na.exclude)
anova(mm3.null,mm3)

##Unripe Fruit
mm4.null<-clmm(unripe~Season + (1|tree:spp) + (1|Month)+(1|Green.Year), 
               data = phen, na.action = na.exclude)
mm4<-clmm(unripe~Drought*Season + (1|tree:spp) + (1|Month)+(1|Green.Year), 
          data = phen, na.action = na.exclude)
anova(mm4.null,mm4)

##Flowers
mm5.null<-clmm(Flowers~Season + (1|tree:spp) + (1|Month)+(1|Green.Year), 
               data = phen, na.action = na.exclude)
mm5<-clmm(Flowers~Drought*Season + (1|tree:spp) + (1|Month)+(1|Green.Year), 
          data = phen, na.action = na.exclude)
anova(mm5.null,mm5)
...
phen.sub<-subset(phen,Year>2011)
phen.subA<-subset(phen.sub,YrAfter=="No")
phen.subB<-subset(phen.sub,YrAfter=="Yes")
phen.sub<-rbind(phen.subA,phen.subB)

##Young Leaves
mm1<-clmm(YL~Season*YrAfter + (1|tree:spp) + (1|Month) + (1|Green.Year), 
          data = phen.sub, na.action = na.exclude)
mm1B<-clmm(YL~Season + (1|tree:spp) + (1|Month) + (1|Green.Year), 
           data = phen.sub, na.action = na.exclude)
anova(mm1,mm1B)

##Mature Leaves
yrm2<-clmm(ML~Season*YrAfter + (1|tree:spp) + (1|Month) + (1|Green.Year), 
           data = phen.sub, na.action = na.exclude)
yrm2.null<-clmm(ML~Season + (1|tree:spp) + (1|Month) + (1|Green.Year), 
                data = phen.sub, na.action = na.exclude)
anova(yrm2.null,yrm2)

##Ripe Fruit
yr.mm3<-clmm(Ripe.fruit~Season*YrAfter + (1|tree:spp) + (1|Month) + (1|Green.Year), 
             data = phen.sub, na.action = na.exclude)
yr.mm3.null<-clmm(Ripe.fruit~Season + (1|tree:spp) + (1|Month) + (1|Green.Year), 
                  data = phen.sub, na.action = na.exclude)
anova(yr.mm3.null,yr.mm3)

##Unripe fruit
yr.mm4<-clmm(unripe~Season*YrAfter + (1|tree:spp) + (1|Month) + (1|Green.Year), 
             data = phen.sub, na.action = na.exclude)
yr.mm4.null<-clmm(unripe~Season + (1|tree:spp) + (1|Month) + (1|Green.Year), 
                  data = phen.sub, na.action = na.exclude)
...
fat.m5B<-lmer(Total.Fat~Pregnant2+AgeClass+Drought+(1|Lemur)+(1|Year),data=data1)

##Perform models for separate fat measures (insert into model): 
#Fat.back #Fat.belly #Fat.arm #Fat.hip
fat.m1<-lmer(Fat.back~Pregnant+(1|Lemur)+(1|Year),data=data1)
fat.m2<-lmer(Fat.back~AgeClass+(1|Lemur)+(1|Year),data=data1)
fat.m3<-lmer(Fat.back~Drought+(1|Lemur)+(1|Year),data=data1)
fat.m4<-lmer(Fat.back~Pregnant+AgeClass+(1|Lemur)+(1|Year),data=data1)
fat.m5<-lmer(Fat.back~Pregnant+AgeClass+Drought+(1|Lemur)+(1|Year),data=data1)
fat.m6<-lmer(Fat.back~Pregnant+Drought+(1|Lemur)+(1|Year),data=data1)
fat.m7<-lmer(Fat.back~Pregnant*Drought+AgeClass+(1|Lemur)+(1|Year),data=data1)
...
exp(confint(ML.nb1,method="Wald")) ## confidence interval
